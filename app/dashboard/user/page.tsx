...
import { 
  Coins, ShoppingBag, Trophy, Gift, TrendingUp, 
  Gamepad2, Star, ArrowRight, Sparkles
} from "lucide-react"
import Link from "next/link"

// Mock data
const stats = [
  { label: "Koin Asli Bogor", value: "2,450", icon: Coins, change: "+150 hari ini", color: "#F99912" },
  { label: "Total Pesanan", value: "12", icon: ShoppingBag, change: "3 dalam proses", color: "#64762C" },
  { label: "Quest Points", value: "850", icon: Trophy, change: "Level 5", color: "#3b82f6" },
  { label: "Mystery Box", value: "3", icon: Gift, change: "Siap dibuka!", color: "#dc2626" },
]

const recentOrders = [
  { id: "ORD-001", product: "Lapis Talas Bogor", status: "Dikirim", date: "10 Mar 2026" },
  { id: "ORD-002", product: "Roti Unyil Venus", status: "Diproses", date: "9 Mar 2026" },
  { id: "ORD-003", product: "Asinan Bogor", status: "Selesai", date: "8 Mar 2026" },
]

const featuredProducts = [
  { name: "Lapis Talas Bogor", price: 85000, rating: 4.9, image: "/placeholder.svg" },
  { name: "Roti Unyil Venus", price: 35000, rating: 4.8, image: "/placeholder.svg" },
  { name: "Tauge Goreng", price: 25000, rating: 4.7, image: "/placeholder.svg" },
]
    ...
    <DashboardLayout role="user">
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <Card key={stat.label} className="bg-card/50 backdrop-blur border-[#F99912]/10 hover:border-[#F99912]/30 transition-all duration-300 hover:shadow-[0_0_20px_rgba(249,153,18,0.1)]">
              ...
              <div className="space-y-4">
                {recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors">
                    ...
                    <Badge 
                      variant={order.status === "Selesai" ? "default" : "secondary"}
                      className={order.status === "Selesai" ? "bg-green-500/20 text-green-500" : order.status === "Dikirim" ? "bg-blue-500/20 text-blue-500" : "bg-[#F99912]/20 text-[#F99912]"}
                    >
                      {order.status}
                    </Badge>
                  </div>
              ...
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-green-500/10 border border-green-500/20">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-foreground">Login Harian</span>
                    <Badge className="bg-green-500/20 text-green-500">+50 XP</Badge>
                  </div>
                </div>
                <div className="p-3 rounded-xl bg-muted/30">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-foreground">Review Produk</span>
                ...
                <div className="p-3 rounded-xl bg-muted/30">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-foreground">Belanja UMKM</span>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {featuredProducts.map((product) => (
                <div key={product.name} className="group p-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-all duration-300 hover:shadow-lg cursor-pointer">
                  ...
                  <h3 className="font-medium text-foreground group-hover:text-[#F99912] transition-colors">{product.name}</h3>
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-[#F99912] font-semibold">Rp {product.price.toLocaleString()}</p>
