...
import { Trophy, Gift, Star, Zap, CheckCircle2, Crown, Medal, Award } from "lucide-react"
...
const badges = [
  { icon: Crown, name: "Raja Belanja", color: "from-[#F99912] to-[#C9C9C3]", earned: true },
  { icon: Medal, name: "Petani Pro", color: "from-[#64762C] to-[#424F17]", earned: true },
  { icon: Award, name: "Review Expert", color: "from-[#C9C9C3] to-[#F99912]", earned: false },
  { icon: Trophy, name: "Top 10", color: "from-[#F99912] to-[#64762C]", earned: false },
]
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#F99912]/10 border border-[#F99912]/20 mb-6">
            ...
            <span className="text-sm text-[#F99912] font-medium">Reward System</span>
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 text-balance">
            ...
            <br />
            <span className="text-foreground">Selesaikan Misi, Raih Hadiah</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 backdrop-blur-xl bg-card/60 border border-[#F99912]/10 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-foreground">Misi Harian</h3>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#F99912]/10">
                <Gift className="w-4 h-4 text-[#F99912]" />
                <span className="text-sm font-semibold text-[#F99912]">+160 Poin</span>
              </div>
            </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                      ...
                      <quest.icon className={`w-6 h-6 ${quest.completed ? 'text-foreground' : 'text-[#F99912]'}`} />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <h4 className="font-medium text-foreground">{quest.title}</h4>
                        <span className={`text-sm font-semibold ${quest.completed ? 'text-[#64762C]' : 'text-[#F99912]'}`}>
                          {quest.reward}
                        </span>
                      </div>
            ...
            <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-[#F99912]/10 to-[#64762C]/10 border border-[#F99912]/20">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-foreground mb-1">Mystery Box</h4>
                  <p className="text-sm text-muted-foreground">
                    Belanja min. Rp100.000 untuk membuka Mystery Box dengan hadiah menarik!
                  </p>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="backdrop-blur-xl bg-card/60 border border-[#F99912]/10 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {badges.map((badge, index) => (
                  ...
                  >
                    <div className={`w-10 h-10 mx-auto rounded-xl bg-gradient-to-br ${badge.color} p-0.5 mb-2`}>
                      <div className="w-full h-full rounded-xl bg-background flex items-center justify-center">
                        <badge.icon className={`w-5 h-5 ${badge.earned ? 'text-[#F99912]' : 'text-muted-foreground'}`} />
                      </div>
                    </div>
            ...
            <div className="backdrop-blur-xl bg-card/60 border border-[#F99912]/10 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/leaderboard" className="text-sm text-[#F99912] hover:underline">
                  Lihat Semua
                </Link>
              </div>
                ...
                {leaderboard.map((user) => (
                  ...
                  >
                    <span className={`w-6 text-center font-bold ${
                      user.rank === 1 ? 'text-[#F99912]' : 
                      user.rank === 2 ? 'text-[#C9C9C3]' : 
                      user.rank === 3 ? 'text-[#64762C]' : 'text-muted-foreground'
                    }`}>
                      {user.rank}
                    </span>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                      user.badge === 'gold' ? 'bg-gradient-to-br from-[#F99912] to-[#C9C9C3] text-[#181612]' :
                      user.badge === 'silver' ? 'bg-gradient-to-br from-[#C9C9C3] to-[#a8a8a3] text-[#181612]' :
                      user.badge === 'bronze' ? 'bg-gradient-to-br from-[#64762C] to-[#424F17] text-foreground' :
                      'bg-muted text-muted-foreground'
                    }`}>
                      {user.avatar}
                    </div>
                    <span className="flex-1 text-sm font-medium text-foreground">{user.name}</span>
                    <span className="text-sm text-[#F99912] font-semibold">{user.points.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
