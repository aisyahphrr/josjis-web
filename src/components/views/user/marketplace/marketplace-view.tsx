...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    const fetchMarketplaceData = async () => {
      try {
        const response = await fetch("/api/umkm/approved");
        if (!response.ok) throw new Error("Failed to fetch marketplace data");

        const { data: umkmList } = await response.json();

        // Transform UMKM + products into products array with seller info
        const allProducts: Product[] = [];
        const categorySet = new Set<string>(["Semua"]);

        umkmList.forEach((umkm: UmkmData) => {
          categorySet.add(umkm.category);

          umkm.products.forEach((product) => {
            allProducts.push({
              id: product.id,
              name: product.name,
              sellerName: umkm.businessName,
              category: umkm.category,
              price: product.priceInIdr,
              rating: 4.5, // Default, bisa dari API jika ada data rating
              reviewsCount: Math.floor(Math.random() * 50) + 10,
              originalPrice: product.priceInIdr * 1.1, // Contoh diskon
            });
          });
        });
    ...
    <div className="space-y-6">
      ...
      >
        {filteredProducts.map((product) => (
          <Card
            key={product.id}
            className={`group bg-card/50 backdrop-blur border-[#F99912]/10 hover:border-[#F99912]/30 transition-all duration-300 hover:shadow-[0_0_30px_rgba(249,153,18,0.1)] overflow-hidden cursor-pointer ${
