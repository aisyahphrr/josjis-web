...
import { type UserRecord } from "@/src/lib/dummyData";
...
export default function AdminDriverPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (filterStatus === "active")
        matchFilter = r.approvalStatus === "approved";
      else if (filterStatus === "pending")
        matchFilter = r.approvalStatus === "pending";
      else if (filterStatus === "rejected")
        matchFilter = r.approvalStatus === "rejected";
  ...
  const handleApproveDriver = async (driverId: string, driverName: string) => {
    try {
      setIsApproving(true);
      const response = await fetch(`/api/admin/driver/${driverId}/approve`, {
        method: "POST",
      });

      if (!response.ok) {
        let errorMessage = "Failed to approve driver";
        try {
      ...
      setRows((prev) =>
        prev.map((x) =>
          x.id === driverId
            ? { ...x, approvalStatus: "approved", status: "active" }
            : x,
        ),
      );
      toast.success(`✓ Berhasil approve: ${driverName}`);
      ...
      toast.error(
        `Failed to approve driver: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
  ...
  const handleRejectDriver = async (
    ...
    try {
      setIsRejecting(true);
      const response = await fetch(`/api/admin/driver/${driverId}/reject`, {
        method: "POST",
      ...
      setRows((prev) =>
        prev.map((x) =>
          x.id === driverId
            ? { ...x, approvalStatus: "rejected", status: "suspended" }
            : x,
        ),
      );
      toast.error(`✗ Rejected: ${driverName}`);
      ...
      toast.error(
        `Failed to reject driver: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Kelola akun driver: lihat status, approve, atau tolak pendaftaran."
        actions={
          ...
          {selectedDriver?.approvalStatus === "pending" && (
            <div className="flex justify-end gap-3 p-6 pt-4 border-t border-border/60 bg-muted/10">
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedDriver) {
                    handleApproveDriver(selectedDriver.id, selectedDriver.name);
                  }
                }}
                disabled={isApproving || isRejecting}
              >
                {isApproving ? "Loading..." : "Terima Pendaftaran"}
              </Button>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="max-w-md rounded-2xl p-0 overflow-hidden">
          ...
          <div className="flex justify-end gap-3 p-6 pt-4 border-t border-border/60 bg-muted/10">
            ...
            >
              {isRejecting ? "Loading..." : "Konfirmasi Tolak"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
            ...
            <Button
              ...
              onClick={() => {
                if (editingDriver) {
                  setRows((prev) =>
                    prev.map((x) =>
                      x.id === editingDriver.id ? editingDriver : x,
                    ),
                  );
                  setEditOpen(false);
