...
import { Check, Search, X, Eye, FileText, Package, User, MapPin, Phone, Mail, Store, Tag, Pencil } from "lucide-react"
...
export default function ValidasiUmkmPage() {
  ...
  const updateStatus = (id: string, next: UmkmApprovalStatus, reason?: string) => {
    setItems((prev) =>
      prev.map((it) => (it.id === id ? { ...it, approvalStatus: next } : it)),
    )
    if (selectedUmkm?.id === id) {
      setSelectedUmkm((prev) => prev ? { ...prev, approvalStatus: next } : null)
    }
  }
  ...
  const handleSaveEdit = async () => {
    ...
    setItems((prev) =>
      prev.map((it) => (it.id === selectedEditUmkm.id ? { ...it, ...editForm } as UmkmRecord : it)),
    )
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/30 hover:bg-muted/30">
                  <TableHead className="py-4 pl-6 text-foreground font-semibold">Registran UMKM</TableHead>
                  <TableHead className="py-4 text-foreground font-semibold">Produk & Kategori</TableHead>
                  <TableHead className="py-4 text-foreground font-semibold">Status Pengajuan</TableHead>
                  ...
                  filtered.map((umkm) => (
                    <TableRow key={umkm.id} className="hover:bg-muted/20 border-b border-border/40">
                      <TableCell className="py-4 pl-6">
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">{umkm.id}</div>
                      </TableCell>
                      <TableCell className="py-4">
                        <div className="font-medium text-sm text-foreground flex items-center gap-1.5">
                          <Tag className="size-3.5" /> {umkm.category}
                        </div>
                        <div className="text-xs text-muted-foreground mt-0.5">{umkm.productCount} produk terdaftar</div>
                      </TableCell>
                      <TableCell className="py-4">
                      ...
                      <TableCell className="py-4 text-right pr-6">
                        <div className="inline-flex items-center justify-end gap-2 w-full">
                          ...
                          >
                            <Eye className="mr-1.5 size-3.5 text-muted-foreground" /> Detail & Validasi
                          </Button>
                        </div>
                      </TableCell>
                    </TableRow>
        ...
        <DialogContent className="max-w-4xl rounded-3xl p-0 overflow-hidden flex flex-col max-h-[90vh] border-border/60 shadow-xl">
          <DialogHeader className="p-6 md:p-8 pb-5 bg-muted/20 border-b border-border/60 relative">
            <div className="flex items-center gap-4">
              ...
              <div className="pt-1">
                <DialogTitle className="text-2xl font-bold tracking-tight mb-2">Peninjauan UMKM</DialogTitle>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-muted-foreground uppercase tracking-wider">ID: {selectedUmkm?.id}</span>
                  <StatusBadge tone={selectedUmkm ? toneByStatus[selectedUmkm.approvalStatus] : "warning"}>
              ...
              <div className="space-y-8">
                ...
                <section>
                  <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Store className="size-4 text-muted-foreground" /> Informasi Usaha / Toko
                  </h3>
                  <div className="rounded-2xl border border-border/60 bg-muted/10 p-5 space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pb-4 border-b border-border/60">
                      <div>
                        <div className="text-xs text-muted-foreground font-medium mb-1">Nama Toko</div>
                        <div className="text-sm font-bold text-foreground">{selectedUmkm.name}</div>
                      </div>
                      <div>
                    ...
                    <div>
                      <div className="text-xs text-muted-foreground font-medium mb-1.5 flex items-center gap-1.5"><FileText className="size-3.5" /> Deskripsi Toko & Penjualan</div>
                ...
                <section>
                  <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
                    <User className="size-4 text-muted-foreground" /> Data Penanggung Jawab
                  </h3>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="flex items-start gap-4 rounded-2xl border border-border/60 bg-muted/10 p-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs text-muted-foreground font-medium mb-0.5">Nama Sesuai KTP / Registrasi</div>
                    ...
                    <div className="flex items-start gap-4 rounded-2xl border border-border/60 bg-muted/10 p-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs text-muted-foreground font-medium mb-0.5">Kontak Aktif (WhatsApp)</div>
                    ...
                    <div className="flex items-start gap-4 rounded-2xl border border-border/60 bg-muted/10 p-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs text-muted-foreground font-medium mb-0.5">Email Bisnis Terdaftar</div>
                    ...
                    <div className="flex items-start gap-4 rounded-2xl border border-border/60 bg-muted/10 p-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs text-muted-foreground font-medium mb-0.5">Alamat Tempat Usaha</div>
                ...
                <section>
                  <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
                    <Package className="size-4 text-muted-foreground" /> Validasi Dokumen (Mockup)
                  </h3>
                  <div className="grid gap-5 md:grid-cols-2">
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-muted-foreground">Kartu Tanda Penduduk</span>
                        {selectedUmkm.ktpUrl && <span className="text-xs text-green-500 font-medium">Terlampir</span>}
                      </div>
                      {selectedUmkm.ktpUrl ? (
                        /* eslint-disable-next-line @next/next/no-img-element */
                        <img src={selectedUmkm.ktpUrl} alt="KTP" className="h-56 w-full rounded-2xl object-cover border border-border/60 bg-muted/30 shadow-sm" />
                      ) : (
                        <div className="h-56 w-full rounded-2xl border border-dashed border-border flex items-center justify-center text-sm text-muted-foreground bg-muted/10">Tidak Ada KTP</div>
                      )}
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-muted-foreground">SKU / Izin Usaha / NIB</span>
                        {selectedUmkm.businessLicenseUrl && <span className="text-xs text-green-500 font-medium">Terlampir</span>}
                      </div>
                      {selectedUmkm.businessLicenseUrl ? (
                        /* eslint-disable-next-line @next/next/no-img-element */
                        <img src={selectedUmkm.businessLicenseUrl} alt="Dokumen Usaha" className="h-56 w-full rounded-2xl object-cover border border-border/60 bg-muted/30 shadow-sm" />
                      ) : (
                        <div className="h-56 w-full rounded-2xl border border-dashed border-border flex items-center justify-center text-sm text-muted-foreground bg-muted/10">Tidak Ada Izin Usaha</div>
                      )}
                    </div>
                  </div>
                </section>
          ...
          {selectedUmkm?.approvalStatus === "pending" ? (
            <div className="p-4 md:p-6 bg-muted/30 border-t border-border/60 flex flex-wrap items-center justify-end gap-3 rounded-b-3xl">
              <Button variant="outline" className="h-10 rounded-xl px-5 border-border/60 bg-background" onClick={() => setDetailOpen(false)}>
                Tutup Review
              </Button>
              <RejectDialog
                onReject={(reason) => {
                  updateStatus(selectedUmkm.id, "rejected")
                  toast.error(`Rejected: ${selectedUmkm.name}`)
                  toast(`Reason: ${reason}`, { duration: 5000 })
                }}
                trigger={
                  <Button variant="destructive" className="h-10 rounded-xl px-6">
                    <X className="mr-2 size-4" /> Tolak Ajuan
                  </Button>
                }
              />
              <ConfirmDialog
                ...
                confirmText="Approve Sekarang"
                onConfirm={() => {
                  updateStatus(selectedUmkm.id, "approved")
                  toast.success(`Berhasil divalidasi dan di-Approve: ${selectedUmkm.name}`)
                }}
                trigger={
                  <Button className="h-10 rounded-xl px-6 bg-[#64762C] hover:bg-[#64762C]/90 text-white">
            ...
            <div className="p-4 md:p-5 bg-muted/10 border-t border-border/60 text-center rounded-b-3xl">
              <p className="text-sm font-medium text-muted-foreground">
                Mitra UMKM ini sudah di-<strong className="text-foreground">{selectedUmkm?.approvalStatus}</strong>. Tidak bisa diubah lagi (dummy).
              </p>
            </div>
          )}
        </DialogContent>
